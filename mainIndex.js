...
io.on("connection", (socket) => {
  ...
  socket.on("handleJoinRequest", ({ roomId, socketId, accept, user }) => {
    console.log(roomId, socketId, accept, user);
  ...
  socket.on("getRtpCapabilities", async ({ roomName }, callback) => {
    console.log("getRtpCapabilities", roomName);
  ...
  socket.on("getProducers", (callback) => {
    console.log("getProducers", callback);
    handleGetProducers(socket, callback);
  });

  // Producer connection and production
  socket.on("transport-connect", ({ dtlsParameters }) => {
    console.log("Transport connection established", dtlsParameters);
  ...
  socket.on(
    ...
    async ({ dtlsParameters, serverConsumerTransportId }) => {
      console.log("recv-connect", dtlsParameters);
  ...
  socket.on(
    ...
    async (
      ...
      console.log("consume", rtpCapabilities, remoteProducerId, server);
      await handleConsume(
        socket,
        rtpCapabilities,
        remoteProducerId,
        ...
        listenIps: [
          {
            ...
            announcedIp: "89.116.134.150", // Replace this with your public IP if your application is public-facing
          },
        ],
        enableUdp: true,
        enableTcp: true,
